...
While vehicle position and orientation on the road is a critical input to a controller, it was not explicitly covered as part of this system. A general approach \textbf{TODO: CITE!} to identify the central point on a road surface is edge detection on the road pixels followed by identifying straight line candidates using the Hough transform. 

\begin{easylist}
	& Identify local window of interest in the near ground to vehicle
	& Use edge detection (eg. Canny) to identify road edge pixels inside window of interest
