...
\textbf{TODO: Look for footnote candidates}
\textbf{TODO: Update section references to FULL reference (currently just subsection references)}
...
\textbf{TODO: What is this being developed for? }It should be noted that the developed method does not account for road traffic. The use of this method is envisaged to be automation of tasks in remote areas such as logistics in large properties such as farms. Alternatively this method provides a single camera redundancy which may assist the capability to `limp home' in the event of main sensor system failures in more complex autonomous vehicles.
...
\citet{ipmForLaneTracking} outlined the effectiveness of Inverse Perspective Mapping for lane detection and \citet{canneyAndHoughLanes} outlined a robust lane detection approach using Canny edge detection and the Hough transform.
...
\textbf{TODO: Yuck - fix wording (and section?) The developed system discussed in this report was implemented with OpenCV using custom simulation inputs, validated by live dashcamera footage. Examples in this paper will consider a the approach to a T junction in simulation. --- THESE COULD BE FOOTNOTES???}
...
\textbf{update frequency and resolution of images?}
\textbf{TODO: System designed to work off GPS informed however can use features as `directions'}

\textbf{TODO: Wider camera FOV helps detection on near approach}
...
As noted in section \ref{s:ipm}, IPM transformation has the assumption of a flat plane which will not be the case along many routes. It was assessed that the local area in front of the vehicle can be assumed to be linearised and testing demonstrated negligable affect on the localisation system when route undulations were incorporated. As a result it is assessed that this assumption can be held as feasible in standard driving conditions.
...
\subsection{Local road orientation}
...
The resulting system has specified limitations as discussed in section \textbf{TODO: Ref section} however within those limitations it performs effectively, in particular the route feature tracking. The `simple' approach of feature detection also works well and the bezier curve driving line provides a control system a constant steering angle for feature traversal.
